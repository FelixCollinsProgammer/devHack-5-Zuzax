...
class RPSResultFrame(QFrame):
    def __init__(self, player_choice, computer_choice, result_text):
        ...
        self.setStyleSheet("""
            QFrame {
                ...
                border-radius: 10px;
                background-color: rgba(0, 128, 128, 30);
            }
        """)
        self.initUI()
    ...
    def initUI(self):
        ...
        # Load images for player and computer choices, default to 'question.png' if None
        ...
        player_pixmap = QPixmap(player_image_path).scaled(48, 48, Qt.KeepAspectRatio)
        computer_pixmap = QPixmap(computer_image_path).scaled(48, 48, Qt.KeepAspectRatio)
        ...
        font = QFont("Segoe UI", 12, QFont.Bold)  # Use a modern sans-serif font
...
class RockPaperScissorsGame(QWidget):
    def __init__(self):
        ...
        self.attempts = 0
        self.MAX_ATTEMPTS = 5
        self.results = []  # Store results for each round
        self.player_choice = None
        self.computer_choice = None
        self.choices = ["Rock", "Scissors", "Paper"]
        self.gesture_locked = False # Flag to prevent multiple gesture detections during processing
        self.round_winner = None # Track winner of current round

        self.hand_tracking_thread = None
        self.create_thread()

        # Timer for countdown before each round
        self.countdown_timer = QTimer(self)
        self.countdown_timer.setInterval(1000)  # 1 second interval
        self.countdown_timer.timeout.connect(self.update_countdown)
        self.countdown_number = 3 # Start countdown from 3

        # Timer to delay processing of results and prepare the next round
        self.delay_timer = QTimer(self)
        self.delay_timer.setInterval(2200)  # Delay of 2.2 seconds
        self.delay_timer.timeout.connect(self.prepare_next_round)
        self.delay_timer.setSingleShot(True)  # Only trigger once
        ...
        self.stacked_widget.setCurrentWidget(self.start_screen)  # Start with the start screen
    ...
    def create_thread(self):
        if self.hand_tracking_thread:
            self.hand_tracking_thread.stop()  # Stop the previous thread if it exists
    ...
    def init_start_screen(self):
        ...
        start_screen.setObjectName("start_screen")  # Set object name for styling
        layout = QVBoxLayout()
        ...
        title_label.setFont(QFont("Impact", 64, QFont.Bold))  # Use a bold, impactful font
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("color: #008080;")  # Teal color
        self.animate_widget(title_label, duration=750, easing_curve=QEasingCurve.OutBounce)  # Animation
        layout.addWidget(title_label)

        # Subtitle Label
        subtitle_label = QLabel("против Компьютера")
        subtitle_label.setFont(QFont("Arial", 24, QFont.Bold))
        ...
        nickname_label.setFont(QFont("Arial", 16))
        nickname_label.setStyleSheet("color: white;")
        self.nickname_input = QLineEdit()
        self.nickname_input.setFont(QFont("Arial", 16))
        ...
        nickname_layout.setAlignment(Qt.AlignCenter)
        ...
        self.add_shadow(self.start_button)  # Add shadow for depth
        ...
        rules_text = (
            ...
            "  - Бумага накрывает Камень<br><br>"  # Added extra spaces for formatting
        ...
        rules_label.setContentsMargins(20, 10, 20, 20)  # Add some padding
    ...
    def check_nickname(self):
        # Enable the start button only if the nickname is not empty
    ...
    def paintEvent(self, event):
        ...
        if self.stacked_widget.currentWidget() == self.start_screen:
            # Radial gradient for the start screen
            gradient = QRadialGradient(rect.center(), min(rect.width(), rect.height()) / 2)
            gradient.setColorAt(0, QColor("#333333"))  # Darker gray
            gradient.setColorAt(1, QColor("#222222"))  # Even darker gray
            painter.setBrush(gradient)

        elif self.stacked_widget.currentWidget() == self.game_screen:
            # Linear gradient for the game screen
            ...
            # Linear gradient for results screen
            gradient = QLinearGradient(0, 0, self.width(), self.height())
            gradient.setColorAt(0, QColor("#222222"))
            gradient.setColorAt(1, QColor("#333333"))
            painter.setBrush(QBrush(gradient))
    ...
    def init_game_screen(self):
        ...
        # Main horizontal layout to arrange player, VS, and computer sections
    ...
    def animate_result(self):
        """Animate the result label's appearance"""
        self.result_label.setWindowOpacity(0)  # Start invisible
        animation = QPropertyAnimation(self.result_label, b"windowOpacity")
        animation.setDuration(500) # 0.5 second animation
    ...
    def start_game(self):
        """Starts the game."""
        self.player_name = self.nickname_input.text().strip()  # Get player's name
        self.player_label.setText(f"Игрок: {self.player_name}")  # Update player label
        self.stacked_widget.setCurrentWidget(self.game_screen)
        self.reset_game_state()  # Reset game state
        if not self.hand_tracking_thread.isRunning():
            self.hand_tracking_thread.start()  # Start the hand tracking thread
    ...
    def update_countdown(self):
        """Updates the countdown label and handles countdown logic."""
        self.countdown_number -= 1
        if self.countdown_number > 0:
            self.countdown_label.setText(str(self.countdown_number))
        else:
            self.countdown_label.setText("")  # Clear the countdown label
            self.countdown_timer.stop()
            self.computer_choice = random.choice(self.choices)  # Computer makes a choice


    def handle_gesture(self, gesture, x, y):
        """Handles the detected gesture."""
        # Ignore gestures if countdown is active or if a gesture has already been locked in
        if self.countdown_timer.isActive() or self.gesture_locked:
            return

        if self.computer_choice is not None: # Only proceed if the computer has made a choice
            self.gesture_locked = True # Lock further gestures
            ...
            self.delay_timer.start()  # Start the delay timer


    def prepare_next_round(self):
        """Prepares the next round or shows the final results."""
        ...
        self.update_score()
        self.round_winner = None  # Reset round winner

        if self.attempts >= self.MAX_ATTEMPTS - 1:  # Check if it's the last round (0-indexed)
            self.show_results()  # Show results screen
        else:
            # Prepare for the next round
            self.attempts += 1
            self.attempts_label.setText(
                f"Раунд: {self.attempts + 1}/{self.MAX_ATTEMPTS}")  # Update attempts label
            self.result_label.setText("") # Clear the previous round result
            self.player_choice_label.clear()  # Clear player choice image
            self.computer_choice_label.clear()  # Clear computer choice image
            self.computer_choice_text_label.setText("") # Clear computer choice text
            self.player_choice = None  # Reset player choice
            self.computer_choice = None  # Reset computer choice
            self.gesture_locked = False  # Unlock gesture detection
            self.start_countdown() # Restart countdown

    def determine_winner(self):
        """Determines the winner of the round."""
        if self.player_choice == self.computer_choice:
            result_text = "Ничья!"
            self.round_winner = "tie"  # Indicate a tie
        ...
        self.animate_result() # Animate result label
        self.results.append((self.player_choice, self.computer_choice, result_text)) # Store the round result
    ...
    def display_player_choice(self):
        """Displays the player's choice as an image."""
        if self.player_choice:
            pixmap = QPixmap(f"{self.player_choice.lower()}.png")  # Load the corresponding image
            pixmap = pixmap.scaled(self.player_choice_label.width(), self.player_choice_label.height(), Qt.KeepAspectRatio)  # Scale the image
            self.player_choice_label.setPixmap(pixmap)
        else:
            self.player_choice_label.clear()  # Clear if no choice

    def display_computer_choice(self):
        """Displays the computer's choice as an image and text."""
        if self.computer_choice:
            pixmap = QPixmap(f"{self.computer_choice.lower()}.png")  # Load image
            pixmap = pixmap.scaled(self.computer_choice_label.width(), self.computer_choice_label.height(),
                                   Qt.KeepAspectRatio)  # Scale image
            self.computer_choice_label.setPixmap(pixmap)
            self.computer_choice_text_label.setText(
                self.computer_choice)  # Display computer's choice as text
        else:
            self.computer_choice_label.clear()  # Clear image
            self.computer_choice_text_label.clear()  # Clear text

    def show_results(self):
        """Displays the final results of the game."""
        self.stacked_widget.setCurrentWidget(self.results_screen)  # Switch to results screen
        ...
        if self.player_score > self.computer_score:
            self.final_result_label.setText(
                f"{self.player_name} побеждает в игре!")  # Player wins
        elif self.computer_score > self.player_score:
            self.final_result_label.setText("Компьютер побеждает в игре!")  # Computer wins
        else:
            self.final_result_label.setText(
                "В игре ничья!")  # It's a tie
    ...
    def reset_game_state(self):
        ...
        self.update_score()  # Reset score display
        self.attempts_label.setText("Раунд: 1/5")  # Reset attempts display
        self.result_label.setText("")  # Clear result label

    def restart_game(self):
        """Restarts the game."""
        self.reset_game_state()  # Reset all game variables
        self.create_thread()  # Restart HandTracking Thread.
        self.stacked_widget.setCurrentWidget(self.start_screen)  # Go back to the start screen
    ...
    def update_image(self, q_img):
        ...
        pixmap = pixmap.scaled(self.video_label.width(), self.video_label.height(), Qt.KeepAspectRatio)  # Scale
    ...
    def closeEvent(self, event):
        ...
        if self.hand_tracking_thread:
            self.hand_tracking_thread.stop()  # Stop the hand tracking thread
        self.countdown_timer.stop()  # Stop the countdown timer
        self.delay_timer.stop()  # Stop the delay timer
        event.accept()
...
if __name__ == "__main__":
    ...
    try:
        ...
        game.showFullScreen()  # Show in full-screen mode
        sys.exit(app.exec_())
    except Exception as main_error:
        print(f"Unhandled exception in main application: {main_error}")  # Error handling
        import traceback
        traceback.print_exc()
        sys.exit(-1)